...
Profile load_profile(const std::string & yaml_path)
{
  ...
  p.thresholds.resupply_total_timeout_s  = get_or<double>(th, "resupply_total_timeout_s", def.resupply_total_timeout_s);
  p.thresholds.retreat_timeout_s   = get_or<double>(th, "retreat_timeout_s", def.retreat_timeout_s);
  p.thresholds.retreat_total_timeout_s   = get_or<double>(th, "retreat_total_timeout_s", def.retreat_total_timeout_s);
  p.thresholds.referee_stale_timeout_s = get_or<double>(th, "referee_stale_timeout_s", def.referee_stale_timeout_s);
  p.thresholds.supply_retry_cooldown_s = get_or<double>(th, "supply_retry_cooldown_s", def.supply_retry_cooldown_s);

  // --- routes & waypoints -------------------------------------------
  p.patrol          = parse_route(root["patrol"]);
  p.patrol_aggressive = parse_route(root["patrol_aggressive"]);
  p.patrol_late     = parse_route(root["patrol_late"]);
