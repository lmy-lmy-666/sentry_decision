...
//
// ROS 2 node — bridges referee/odom data → Context → BehaviourTree → Nav2 goals.
// Behaviour-equal to the FSM sample's node; the decision engine is BtDecision.
//
#include "omni_behavior_sample/decision_node.hpp"
...
: rclcpp::Node("omni_behavior_sample", options)
{
  // --- parameters --------------------------------------------------
  ...
  if (profile_path.empty()) {
    throw std::runtime_error("omni_behavior_sample: 'profile_path' parameter is required");
  }
  ...
  // --- behaviour tree ----------------------------------------------
  bt_ = std::make_unique<BtDecision>(
    std::move(profile),
  ...
  RCLCPP_INFO(get_logger(), "omni_behavior_sample started @ %.1f Hz", tick_hz);
}
...
void DecisionNode::on_tick()
{
  ...
  // Log branch transitions
  if (current != last_logged_state_) {
    RCLCPP_INFO(
      get_logger(), "branch -> %-8s | reason: %-35s | hp=%3u/%-3u ammo=%3u remain=%3ds nav=%d",
      to_string(current), bt_->state_reason(),
...
RCLCPP_COMPONENTS_REGISTER_NODE(omni_behavior_sample::DecisionNode)
