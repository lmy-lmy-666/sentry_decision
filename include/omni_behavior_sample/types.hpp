...
//
// Sentry decision types — navigation-only, no combat.
// (Shared verbatim with the FSM sample; the behavior-tree port reuses these
//  domain types unchanged so the two implementations stay bit-for-bit equal.)
//
#ifndef OMNI_BEHAVIOR_SAMPLE__TYPES_HPP_
#define OMNI_BEHAVIOR_SAMPLE__TYPES_HPP_
...
/// Top-level behaviours — keep it minimal.
...
///
/// In the behavior-tree port these are NOT a state variable that gates
/// transitions; they are the *label* of whichever branch of the tree ran this
/// tick. The tree re-decides from scratch every tick (reactive), and the label
/// is recorded purely for logging/telemetry parity with the FSM sample.
enum class State {
...
struct Thresholds
{
  ...
  uint8_t min_ticks_in_state{4};   ///< min ticks before a branch switch (oscillation guard)
...
#endif  // OMNI_BEHAVIOR_SAMPLE__TYPES_HPP_
