...
def _make_node(context, *, pkg_share, default_profile):
    ...
    return [
        Node(
            package="omni_behavior_sample",
            executable="omni_behavior_sample_node",
            name="omni_behavior_sample",
            output="screen",
            parameters=[{
                # 战术配置文件（YAML）：巡逻点、补给点、开局站位、起伏段等
                "profile_path": profile,

                # 行为树主循环频率 (Hz)，每隔 1/tick_frequency 秒 tick 一次整棵树
                "tick_frequency": 10.0,
...
def generate_launch_description():
    pkg_share = get_package_share_directory("omni_behavior_sample")
